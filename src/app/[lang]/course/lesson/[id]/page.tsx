  ...
  return (
    <div className="max-w-3xl mx-auto container py-8">
      ...
      <div className="flex justify-between mt-8">
        {prevLesson ? (
          <Button asChild variant="outline">
            <Link href={`/${params.lang}/course/lesson/${prevLesson.id}`}><ArrowLeft className="mr-2 h-4 w-4" /> Previous Lesson</Link>
          </Button>
        ) : <div />}
        {nextLesson ? (
          <Button asChild>
            <Link href={`/${params.lang}/course/lesson/${nextLesson.id}`}>Next Lesson <ArrowRight className="ml-2 h-4 w-4" /></Link>
          </Button>
        ) : <div />}
      </div>
    </div>
